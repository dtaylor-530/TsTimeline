    ...
    public partial class App
    {
        ...
        void reloadData()
        {
            ...
            else if (chartTypeViewModel.Enum is ChartType.Map)
            {
                ...
                foreach (var child in progressX.FindChildren<ViewModel>(Keys.TimelineThumb))
                {
                    child.Direction = Direction.None;
                    child.Visibility = Visibility.Collapsed;
                }
                ...
                foreach (var child in progressY.FindChildren<ViewModel>(Keys.TimelineThumb))
                {
                    child.Direction = Direction.None;
                    child.Visibility = Visibility.Collapsed;
                }
        ...
        void initialiseViewPorts()
        {
            ...
            void propertyChanged(TreeView treeView, Viewport viewport, string name, List<TreeViewItem> _list)
            {
                ...
                void refresh(Viewport viewport)
                {
                    ...
                    {
                        if (item is ClipBase { DataContext: { } context } clipBase)
                        {
                            var updateContext = new UpdateContext(clipBase, viewport);

                            if (getUpdater(updateContext) is { } updater)
                            {
            ...
            if (sender is FrameworkElement { DataContext: ViewModel { } viewmodel } element)
                if (viewmodel.Parent.FindChild<ViewModel>(Keys.Playlist) is { } playlist)
                {
                    ...
                    if (viewmodel.Group == Groups.One)
                        ...
                        {
                            var context = new RenderContext(element, viewport, e.DrawingContext, playlist);
                            if (getUpdater(context) is { } updater)
            ...
            new Action(() =>
            {
                ...
                if (clipBase.DataContext is Notification { } notification)
                {
                    if (notification.Group == Groups.One)
                    {
                        ...
                        foreach (var viewport in GroupOneViewports)
                        {
                            var context = new InitialisationContext(clipBase, viewport);
                            if (getUpdater(context) is { } updater)
                                updater.Update(context);
                        }
                    }
                    else if (notification.Group == Groups.Two)
                    {
                        ...
                        foreach (var viewport in GroupTwoViewports)
                        {
                            var context = new InitialisationContext(clipBase, viewport);
                            if (getUpdater(context) is { } updater)
                                updater.Update(new InitialisationContext(clipBase, viewport));
                        }
                    }
                }
                else
        ...
        List<IUpdater> updaters = [new Updater(), new CountryUpdater()];

        IUpdater getUpdater(object context)
        {
            return updaters.SingleOrDefault(u => u.CanUpdate(context));
