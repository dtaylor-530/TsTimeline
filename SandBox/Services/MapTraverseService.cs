                ...
                foreach (var line in lines)
                {
                    if (line.Axis == Axis.X)
                        if (xViewport.Offset < xViewport.Length / 2)
                            line.X = xViewport.Offset;
                        else if (xViewport.Offset > maxX - xViewport.Length / 2)
                            line.X = xViewport.Offset - maxX + xViewport.Length / 2;
                        else
                            line.X = xViewport.Length / 2;

                    else if (line.Axis == Axis.Y)
                        if (yViewport.Offset < yViewport.Length / 2)
                            line.Y = yViewport.Offset;
                        else if (yViewport.Offset > maxY - yViewport.Length / 2)
                            line.Y = yViewport.Offset - maxY + yViewport.Length / 2;
                        else
                            line.Y = yViewport.Length / 2;
                }
            });
